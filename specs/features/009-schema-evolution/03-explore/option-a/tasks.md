...
| TASK-A-002 | Run a dry-run preview SELECT: `SELECT device, SUBSTRING_INDEX(device, '/', -2) AS location, SUBSTRING_INDEX(device, '/', -1) AS measurement_type, TIMESTAMP(currentdate, currenttime) AS captured_at FROM sensorreadings LIMIT 50` — inspect results for correctness across a representative sample of device topics | Analysis | Chris | 15 min | IP-001 | Confirmed backfill derivation is correct for all observed topic patterns |
